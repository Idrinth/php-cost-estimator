...
class UsesStaticComparison implements Rule
{
    ...
    public function applies(Node $astNode): bool
    {
        if ($astNode instanceof Node\Expr\FuncCall && $astNode->name instanceof Node\Name && $astNode->name->toLowerString() === 'in_array') {
            if ($astNode->args[0]->value instanceof Node\Scalar && $astNode->args[1]->value instanceof Node\Expr\Array_) {
                return true;
            }
            if ($astNode->args[0]->value instanceof Node\Expr\ConstFetch && $astNode->args[1]->value instanceof Node\Expr\Array_) {
                return true;
            }
